    ...
    @foreach ($apprenants as $apprenant)
        <div class="card" style="width: 18rem;">
            <div class="card-body">
                ...
                <form action="{{ route('assigner.destroy',$apprenant->id)}}" method="post">
                    @csrf
                    @method('delete')
                ...
                {{-- <a href="{{route ('assigner.destroy',$apprenant->id)}}" class="card-link"><button type="button" class="btn btn-outline-success">desassigner</button></a> --}}
            </div>
        </div>
    @endforeach
</div>
